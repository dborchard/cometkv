...
import (
	...
	time2 "memtable/pkg/common/timestamp"
	"runtime"
	"time"
)
...
type EMBase struct {
	TTL      time.Duration
	derived  IEphemeralMemtable
	moAvg    *movingaverage.MovingAverage
	logStats bool
}

func NewBase(bt IEphemeralMemtable, gc, ttl time.Duration, logStats bool) *EMBase {
	return &EMBase{
		derived:  bt,
		TTL:      ttl,
...
func (e *EMBase) StartGc(interval time.Duration, ctx context.Context) {
	...
	for {
		...
		case <-ticker.C:
			...
			e.Prune(time2.ToUnit64(expiredTs))
		case <-ctx.Done():
			return
		}
	}
...
func (e *EMBase) Scan(k string, c int, Ts time.Time) []Pair[string, []byte] { panic("not implemented") }
