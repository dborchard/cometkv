...
type SkipList[K, V any] interface {
	...
	Scan(key K, iter func(element *Element[K, V]) bool)
}
...
// Scan returns a scannable interface for scanning all the records
// TODO: Add more desc
func (list *skipListUnSafe[K, V]) Scan(key K, iter func(element *Element[K, V]) bool) {
	item := list.Find(key)
