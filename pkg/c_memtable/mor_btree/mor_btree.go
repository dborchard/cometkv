...
func New(gcInterval, ttl time.Duration, logStats bool, ctx context.Context) common.IEphemeralMemtable {
	sr := MoRBTree{segmentDuration: gcInterval}
	...
	sr.base = common.NewBase(&sr, gcInterval, ttl, logStats)
...
func (s *MoRBTree) Put(key string, val []byte) {
	s.Lock()
	defer s.Unlock()
	//1. Find curr segment
	currTs := time.Now()
...
func (s *MoRBTree) Close() {
	for _, segment := range s.segments {
		segment.Clear()
	}
}

func (s *MoRBTree) findSegmentIdx(snapshotTs time.Time) int {
	cycleOffset := float64(snapshotTs.Unix() % s.cycleDuration)
	segmentIdx := int(math.Floor(cycleOffset / s.segmentDuration.Seconds()))
	return segmentIdx
}

func (s *MoRBTree) Get(key string, snapshotTs time.Time) []byte {
	return s.base.Get(key, snapshotTs)
}

func (s *MoRBTree) Delete(key string) {
	s.base.Delete(key)
}

func (s *MoRBTree) StartGc(interval time.Duration, ctx context.Context) {
